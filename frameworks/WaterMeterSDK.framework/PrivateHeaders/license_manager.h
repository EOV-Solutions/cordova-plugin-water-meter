...
public:
    ...
    /**
     * Set the secret key for JWT verification (called from Swift with obfuscated key)
     */
    void setSecretKey(const std::string& key);
    
    /**
     * Get the hardcoded API base URL (security: cannot be modified from Swift)
     */
    ...
    /**
     * Set blocked state (called from Swift after sync attempt)
    ...
    /**
     * Set last sync time (called from Swift with stored value on startup)
...
private:
    // Hardcoded API URL for security - cannot be changed from Swift layer
    static constexpr const char* BASE_URL = "http://10.0.0.237:8000";
    
    // Hardcoded default secret key for security - cannot be read from decompiled app
    // IMPORTANT: Change this in production!
    static constexpr const char* DEFAULT_SECRET_KEY = "CHANGE_THIS_SECRET_KEY_IN_PRODUCTION";
    
    // Hardcoded quota threshold for security - cannot be changed from Swift layer
    static constexpr int QUOTA_THRESHOLD = 2000;
    
    // Hardcoded sync interval for security - 2 hours in milliseconds (for testing)
