...
@property(nonatomic, assign)
    BOOL isScannerPresented; // Guard against multiple calls
...
- (void)initialize:(CDVInvokedUrlCommand *)command {
  [self.commandDelegate runInBackground:^{
    ...
    NSInteger threadCount =
        options[@"threadCount"] ? [options[@"threadCount"] integerValue] : 2;
    BOOL useGPU = options[@"useGPU"] ? [options[@"useGPU"] boolValue] : NO;
    NSInteger maxSideLen = options[@"maxSideLength"]
                               ? [options[@"maxSideLength"] integerValue]
                               : 480; // Model trained with 480x480

    // Create configuration (use ObjC wrapper)
    WMPredictorConfiguration_ObjC *config =
        [[WMPredictorConfiguration_ObjC alloc] initWithThreadCount:threadCount
...
- (void)initializeLicense:(CDVInvokedUrlCommand *)command {
  [self.commandDelegate runInBackground:^{
    ...
    // Parse optional metadataInfo (second argument)
    NSDictionary *metadataInfo = nil;
    if (command.arguments.count > 1 &&
        [command.arguments[1] isKindOfClass:[NSDictionary class]]) {
      metadataInfo = command.arguments[1];
    }

    // Parse optional deviceUser (third argument)
    NSString *deviceUser = nil;
        ...
        initializeLicenseWithLicenseKey:licenseKey
                           metadataInfo:metadataInfo
                             ...
                             completion:^(BOOL success,
                               ...
                               CDVPluginResult *result;
                               NSInteger status =
                                   [WaterMeterSDK shared].licenseStatus;

                               if (success) {
                                 self.licenseInitialized = YES;
                                 NSLog(@"[WaterMeter Plugin] License "
                                 ...
                                 result = [CDVPluginResult
                                     ...
                                     messageAsDictionary:@{
                                       ...
                                       @"status" : @(status),
                                       @"message" :
                                           [self statusMessageForCode:status]
                                     }];
                               } else {
                                 ...
                                 NSLog(@"[WaterMeter Plugin] License error: %@",
                                       error);
                                 result = [CDVPluginResult
  ...
  [self.commandDelegate runInBackground:^{
    ...
    if (!image) {
      CDVPluginResult *result = [CDVPluginResult
          resultWithStatus:CDVCommandStatus_ERROR
           messageAsString:
               [NSString stringWithFormat:@"Could not load image from: %@",
                                          filePath]];
      [self.commandDelegate sendPluginResult:result
                                  callbackId:command.callbackId];
      return;
  ...
  [self ensureInitializedWithCompletion:^(NSError *initError) {
    if (initError) {
      CDVPluginResult *result = [CDVPluginResult
          resultWithStatus:CDVCommandStatus_ERROR
           messageAsString:
               [NSString stringWithFormat:@"SDK initialization failed: %@",
    ...
    // Perform recognition
    NSError *error = nil;
    WMOCRScanResult_ObjC *ocrResult =
        [[WaterMeterSDK shared] recognizeWithImage:image error:&error];
...
- (void)scan:(CDVInvokedUrlCommand *)command {
  ...
  BOOL showCloseButton = options[@"showCloseButton"]
                             ? [options[@"showCloseButton"] boolValue]
                             : YES;

  // Image resize options (like Android)
  NSInteger imageMaxWidth =
      options[@"imageMaxWidth"] ? [options[@"imageMaxWidth"] integerValue] : 0;
  NSInteger imageMaxHeight = options[@"imageMaxHeight"]
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (!self.viewController.view.window) {
      NSLog(@"[WaterMeter Plugin] scan: DELAYED - view not in hierarchy, "
            @"retrying in 0.3s");
      dispatch_after(
...
- (void)presentScannerWithOptions:(NSDictionary *)options
  ...
  [self ensureInitializedWithCompletion:^(NSError *initError) {
    ...
    // Present scanner
    NSError *error = nil;
    NSLog(@"[WaterMeter Plugin] presentScanner about to call SDK (using SDK "
    ...
    if (error) {
      self.isScannerPresented = NO;
      NSLog(@"[WaterMeter Plugin] presentScanner ERROR: %@",
...
- (void)scanner:(WMCameraScanner *)scanner
    didScanResult:(WMScanResult_ObjC *)result {
  // SDK already dismisses the view controller, so we just send the result
  // directly Do NOT call dismissViewControllerAnimated here - it will cause
  // completion to never fire
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    NSLog(@"[WaterMeter Plugin] Sending plugin result to JS callback: %@",
          self.scanCallbackId);
...
- (void)scanner:(WMCameraScanner *)scanner didFailWithError:(NSError *)error {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    self.isScannerPresented = NO; // Reset guard flag
    NSLog(@"[WaterMeter Plugin] didFailWithError - isScannerPresented = NO, "
          @"error: %@",
          error.localizedDescription);
    CDVPluginResult *result =
        [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
...
- (void)openSettings:(CDVInvokedUrlCommand *)command {
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    WMSettingsViewController *settingsVC =
        [[WMSettingsViewController alloc] init];
...
- (NSDictionary *)ocrResultToDictionary:(WMOCRScanResult_ObjC *)ocrResult {
  ...
  if (ocrResult.imagePath && ocrResult.imagePath.length > 0) {
    ...
    if (imageData) {
      ...
      dict[@"imageBase64"] = [NSString
          stringWithFormat:@"data:image/jpeg;base64,%@", base64String];
    }
  }
...
- (NSString *)statusMessageForCode:(NSInteger)status {
  ...
  case 5:
    return @"License has been blocked";
  case 6:
    return @"Quota exceeded, sync required";
  default:
